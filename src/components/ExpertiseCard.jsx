...
const ExpertiseCard = ({ icon: Icon, title, description, isDark = true }) => {
    return (
        <motion.div
            ...
            className={`${isDark ? 'bg-[#1a2544]/80 border-gray-700' : 'bg-white/80 border-gray-200'} 
                backdrop-blur-sm p-6 rounded-xl border transition-colors duration-300`}
        >
            <div className="flex items-center gap-4 mb-4">
                <div className={`p-3 ${isDark ? 'bg-[#293556]' : 'bg-blue-50'} rounded-lg transition-colors duration-300`}>
                    <Icon className={`w-6 h-6 ${isDark ? 'text-[#70C8F2]' : 'text-blue-500'} transition-colors duration-300`} />
                </div>
                <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} transition-colors duration-300`}>
                    {title}
                </h3>
            </div>
